...
namespace Assets.scripts.Presentation.MenuScene
{
    internal class SettingsTab : MonoBehaviour
    {
        ...
        {
            GameSettings.Instance.FirstMoveTurn = _firstMoveTurnDropdown.value == 0 ? OpponentType.Player : OpponentType.AI;
            GameSettings.Instance.BoardHeight = int.Parse(_boardHeightInputField.text);
            GameSettings.Instance.BoardWidth = int.Parse(_boardWidthInputField.text);
            GameSettings.Instance.AISearchDeep = int.Parse(_aiSearchDeepInputField.text);
            GameSettings.Instance.IsGiveaways = _isGiveawaysToggle.isOn;
            GameSettings.Instance.OpponentCountOfChechers = int.Parse(_countOfCheckersPerOpponentInputField.text);
