...
namespace Assets
{
    ...
    {
        public const int SEARCH_DEEP = 8;
        ...
        public const int PROXIMITY_END_OF_BOARD_COUNT_OF_POINTS = KING_CHECKER_COUNT_OF_POINTS;
        public const int COUNT_OF_POINTS_PER_MOVE = 2;
        ...
        void Update()
        {
            if (Game.IsPaused ||
                ...
                !BoardEntities.Instance.IsAllCheckersMoved ||
                IsCalculating) return;
        ...
        private async Task<(float points, CheckerMove? move)> EvaluateTree(BoardPosition pos, bool isMax, float alpha, float betta, int curDeep = 0)
        {
            ...
            if (curDeep == SEARCH_DEEP || (isMax && !pos.IsOpponentCanMove(OpponentType.AI)) || (!isMax && !pos.IsOpponentCanMove(OpponentType.Player)))
            ...
            if (curDeep == 0)
            {
                ...
                for (int i = 0; i < moves.Count; i++)
                {
                    ...
                    nextPosition.MakeMove(nextPosition.Data[moves[i].From.x, moves[i].From.y], moves[i].To,
                        out bool isOpponentContinueBeating, out CheckerData beatenChecker, out _);
        ...
        public static float GetPositionAssessment(BoardPosition position)
        {
            ...
            if (position.PlayerCheckerCount == 0) return 1f;
            if (position.AICheckerCount == 0) return 0f;
