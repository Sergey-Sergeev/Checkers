...
namespace Assets
{
    public class CheckersAI : MonoBehaviour
    {
        ...
        private static bool _isAllThreadsExit = false;
        private static bool _isBlocked = false;

        private static List<(Task<(float points, CheckerMove? nextMove)> task, CheckerMove lastMove)> _calculatingThreads = 
        ...
        public static void RestartCalculating()
        {
            var task = StopCalculating();
            if (!task.IsCompleted)            
                task.Wait();
        ...
        public static async Task StopCalculating()
        {
            ...
            Debug.Log($"count threads: {_calculatingThreads.Count}");

            while (_calculatingThreads.Count > 0)
            {
                Debug.Log($"count threads: {_calculatingThreads.Count}");
                var completedTask = await Task.WhenAny(_calculatingThreads.Select(t => t.task));
                var completedItem = _calculatingThreads.First(t => t.task == completedTask);
                _calculatingThreads.Remove(completedItem);
            }
        ...
        {
            if (_isAllThreadsExit) return (0, null);

            CheckerMove? move = null;

            if (curDeep == GameSettings.AISearchDeep || (isMax && !pos.IsOpponentCanMove(OpponentType.AI)) || (!isMax && !pos.IsOpponentCanMove(OpponentType.Player)))
            ...
            {             
                for (int i = 0; i < moves.Count; i++)
                {
                    ...
                    if (_isBlocked) return (0, null);

                    _calculatingThreads.Add((Task.Run(() => EvaluateTree(nextPosition, isOpponentContinueBeating ? isMax : !isMax, alpha, betta, curDeep + 1)), lastMove));
                }
